// 文件: static/js/features/f9-TravelTime.js
...
// window.f9AreaOverlays = []; // 不再需要
// window.f9PathRelatedOverlays = []; // 不再需要

/**
 * F9: 绘制区域A和区域B的矩形及标签。
 * 覆盖物会被添加到 window.allFeatureOverlays 的对应条目中。
 */
...
function drawF9ShortestPathAndStore(pathPoints) {
  ...
  // 全局 clearOverlays 应该已经清除了旧的 F9_路径覆盖物
  // window.allFeatureOverlays["F9_路径覆盖物"] = []; // 确保在绘制前是空的（或者由全局清除保证）
  ...
  // map.setViewport(bmapPoints); // 或者结合区域一起调整
}

// window.clearF9Overlays 函数不再需要，由全局 clearOverlays 通过 allFeatureOverlays 处理
// window.clearF9Overlays = () => { ... }; // 删除此函数
...
function performCommunicationTimeAnalysis(
  ...
  // 原先的 window.clearF9Overlays(); 或其内部逻辑不再需要在这里单独调用。
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // F9 模块加载时，确保它在 window.allFeatureOverlays 中的条目存在 (双重保险)
