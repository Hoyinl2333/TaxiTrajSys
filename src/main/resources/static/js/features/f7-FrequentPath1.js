...
window.pathPolylines = [] // 存储路径折线对象，暴露到全局作用域
...
function performFrequentPathAnalysis1(k, minDistance) {
  ...
  const params = {
    k: Number.parseInt(k, 10), // 假设 k 需要是整数
    minPathDistanceKM: Number.parseFloat(minDistance), // 假设 minDistance 需要是浮点数
  }
...
function processFrequentPathData(data) {
  ...
  if (data && data.pathFrequencies && data.pathFrequencies.length > 0) {
    // 保存数据到全局变量
    window.pathFrequenciesData = data.pathFrequencies
    window.currentPathIndex = 0
...
function displayPathOnMap(pathIndex) {
  if (!window.pathFrequenciesData || pathIndex >= window.pathFrequenciesData.length) {
  ...
  convertCoordinates(points, (data) => {
    if (data.status === 0) {
      ...
      window.pathPolylines.push(polyline)
...
function updatePathDetails(pathIndex) {
  const pathDetails = document.getElementById("path_details")
  if (!pathDetails || !window.pathFrequenciesData || pathIndex >= window.pathFrequenciesData.length) return
