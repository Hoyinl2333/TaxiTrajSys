...
function analyzeDensity() {
    ...
    var resultDiv = document.getElementById("f4_result")
    if (!resultDiv) {
        // 如果f4_result不存在，创建一个
        resultDiv = document.createElement("div")
        resultDiv.id = "f4_result"
        resultDiv.className = "function-result"
        // 添加到功能区域内
        document.getElementById("f4").appendChild(resultDiv)
    }
    ...
    fetch(apiUrl)
       ...
       .then((data) => {
            ...
            // 更新结果信息到功能区域内的结果div
            resultDiv.innerHTML = `<p>密度分析结果：</p><p>成功获取密度数据，共 ${timeSlots.length} 个时间点</p>`

            // 将地图中心设置到分析区域中心
            var centerLng = (Number.parseFloat(data.minLon) + Number.parseFloat(data.maxLon)) / 2
            var centerLat = (Number.parseFloat(data.minLat) + Number.parseFloat(data.maxLat)) / 2
...
function drawGridFromData(data) {
    var minLon = Number.parseFloat(data.minLon)
    var minLat = Number.parseFloat(data.minLat)
    var maxLon = Number.parseFloat(data.maxLon)
    var maxLat = Number.parseFloat(data.maxLat)
    var rows = Number.parseInt(data.rows)
    var cols = Number.parseInt(data.cols)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 时间切换按钮事件
    document.getElementById("prevTimeSlot").addEventListener("click", () => {
        if (timeSlots.length > 0 && currentTimeIndex > 0) {
            updateHeatmap(currentTimeIndex - 1)
        }
    })

    document.getElementById("nextTimeSlot").addEventListener("click", () => {
